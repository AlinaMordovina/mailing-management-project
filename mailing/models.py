...
class Mailing(models.Model):
    ...
    request_period = models.CharField(
        max_length=50, choices=PERIOD_CHOICES, verbose_name="Периодичность"
    )
    status = models.CharField(
        max_length=50, choices=STATUS_CHOICES, default=CREATED, verbose_name="Статус"
    )
    created_at = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата создания рассылки", blank=True
    )
    last_mailing = models.DateField(
    ...
    massage = models.ForeignKey(
        Massage, on_delete=models.CASCADE, verbose_name="Сообщение для рассылки"
    )
    owner = models.ForeignKey(
        User, verbose_name="Владелец", blank=True, null=True, on_delete=models.SET_NULL
    )
...
class Log(models.Model):
    mailing = models.ForeignKey(
        Mailing, on_delete=models.CASCADE, verbose_name="Почтовая рассылка"
    )
    last_try = models.DateTimeField(
        auto_now_add=True, verbose_name="Дата и время последней попытки"
    )
    status = models.BooleanField(verbose_name="Статус попытки")
    mail_service_response = models.CharField(
        max_length=150, verbose_name="Ответ почтового сервиса", blank=True, null=True
    )

    def __str__(self):
        return (
            f"{self.mailing}: последняя попытка {self.last_try}, статус {self.status}"
        )
